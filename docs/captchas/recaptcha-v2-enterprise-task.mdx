...
В данном примере `6LdIFr0ZAAAAAO3vz0O0OQrtAefzdJcWQM2TMYQH` — это значение параметра `websiteKey`.
    ...
    Некоторые реализации reCAPTCHA Enterprise могут содержать дополнительный параметр `s`, передаваемый в `grecaptcha.enterprise.render` вместе с `sitekey`. Например, значение `2JvUXHNTnZl1Jb6WEvbDyB...ugQA`:
