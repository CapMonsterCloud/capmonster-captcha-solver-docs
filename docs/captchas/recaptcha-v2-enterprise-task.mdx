...
Ниже представлены примеры вариантов reCAPTCHA v2 Enterprise, поддерживаемых CapMonster Cloud:
...
## Параметры запроса
  ...
  <div className="bordered-panel">
    ...
    **RecaptchaV2EnterpriseTask**
    ...
    Ключ-идентификатор reCAPTCHA на целевой странице.<br />
	Например, значение `websiteKey` можно найти в HTML-коде страницы:

`<div class="g-recaptcha" data-sitekey="THIS_ONE"></div>`

или в URL атрибута `src` элемента `iframe`:

`<iframe title="reCAPTCHA" src="...;k=6LdIFr0ZAAAAAO3vz0O0OQrtAefzdJcWQM2TMYQH&amp;...">`

В данном примере `6LdIFr0ZAAAAAO3vz0O0OQrtAefzdJcWQM2TMYQH` — это значение параметра `websiteKey`.
    ...
    Объект с дополнительными параметрами reCAPTCHA Enterprise.

    **Параметры объекта:**

    - `s` — `string`, дополнительный параметр reCAPTCHA Enterprise, который может передаваться в `grecaptcha.enterprise.render` вместе с `sitekey`.
...
<br />
Для получения параметра `s` из `enterprisePayload` замените функцию `grecaptcha.enterprise.render` перед её вызовом и извлеките значение из передаваемого объекта параметров. Функция становится доступной после загрузки скрипта, а CAPTCHA обычно рендерится сразу или по событию страницы, поэтому перехват необходимо выполнить заранее. Оригинальную функцию можно вызывать без поля `s`.
