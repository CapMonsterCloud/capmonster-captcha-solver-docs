...
<div className="bordered-panel">
    ...
    **captchaAppId**, соответствует значению `aid`. Например, `"websiteKey": "189123456"` - уникальный параметр для вашего сайта. Можно взять с html страницы с капчей или из трафика (см. описание ниже).
    ...
    Ссылка на скрипт CAPTCHA. Обычно файл называется `TCaptcha.js` или `TCaptcha-global.js`. Для второй версии также встречаются имена вида `TJNCaptcha.js` и `TJCaptcha.js`. Скрипт можно найти среди сетевых запросов (см. пример ниже).
...
Откройте *Инструменты разработчика*, перейдите на вкладку **Network**, активируйте капчу и изучите сетевые запросы. Среди загруженных файлов найдите `TCaptcha.js` или `TCaptcha-global.js`:

![](./images/tendi/tendi_captchaUrl.png)

Для второй версии также встречаются файлы вида `TJNCaptcha.js` или `TJCaptcha.js`:
