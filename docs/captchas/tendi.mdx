...
<div className="bordered-panel">
    ...
    **captchaAppId**, соответствует значению `aid`. Например, `"websiteKey": "189123456"` - уникальный параметр для вашего сайта. Можно взять с html страницы с капчей или из трафика (см. описание ниже).

    ---

    <ParamItem title="captchaUrl (внутри metadata)" type="string" />
    Ссылка на скрипт CAPTCHA. Обычно файл называется `TCaptcha.js` или `TCaptcha-global.js`. Для второй версии также встречаются файлы вида `TJNCaptcha*.js` и `TJCaptcha*.js`, где `*` означает произвольную или пустую строку, например `-global`. Скрипт можно найти среди сетевых запросов (см. пример ниже).
...
Откройте *Инструменты разработчика*, перейдите на вкладку **Network**, активируйте CAPTCHA и изучите сетевые запросы. Среди загруженных файлов найдите `TCaptcha.js` или `TCaptcha-global.js`:

![](./images/tendi/tendi_captchaUrl.png)

Для второй версии также встречаются файлы вида `TJNCaptcha*.js` и `TJCaptcha*.js`:

![](./images/tendi/tendi_captchaUrl-1.png)

`*` в имени файла означает произвольную или пустую строку. Например, шаблон `TJNCaptcha*.js` может соответствовать файлам `TJNCaptcha.js` или `TJNCaptcha-global.js`.
