...
<div className="bordered-panel">
  ...
  **TurnstileTask**
  ...
  Ключ Cloudflare. Допускается передача любой строки, например: `xxxxxx`.
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Cloudflare Waiting Room решается только с использованием вашего прокси
  const proxy = {
    proxyType: "http",
...
async def main():
    ...
    # Cloudflare wait_room решается только с использованием вашего прокси
    proxy = ProxyInfo(
        proxyType="http",
        proxyAddress="123.45.67.89",
        proxyPort=8080,
