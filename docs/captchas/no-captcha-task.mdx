...
Ниже представлены примеры вариантов reCAPTCHA v2, поддерживаемых CapMonster Cloud:
...
## Параметры запроса
  ...
  <div className="bordered-panel">
    ...
    **RecaptchaV2Task**
    ...
    Ключ-идентификатор ReCaptcha2 на целевой странице: `<div class="g-recaptcha" data-sitekey="THIS_ONE"></div>`<br />
    
  Имеет следующий примерный формат: `6Lcg7CMUAAAAANphynKgn9YAgA4tQ2KI_iqRyTwd`
...
Используйте метод [getTaskResult](../api/methods/get-task-result.mdx), чтобы получить решение reCAPTCHA v2. В зависимости от загрузки системы ответ может быть получен через 100 мс.
...
Для некоторых сайтов ответ может выглядеть следующим образом. При отправке решения необходимо использовать UserAgent, указанный в ответе, даже если он отличается от текущего UserAgent браузера или скрипта.
  ...
  <details>
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            {
                WebsiteUrl = "https://yourwebsite.com/page-with-recaptcha", // URL страницы с капчей
