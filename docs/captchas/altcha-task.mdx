...
Ниже представлены примеры типов заданий Altcha, которые на данный момент поддерживаются сервисом CapMonster Cloud:
...
<div className="bordered-panel">
  ...
  **altcha**
  ...
  Уникальный идентификатор задачи, полученный с веб-страницы.<br />
  Пример: `3dd28253be6cc0c54d95f7f98c517e68744597cc6e66109619d1ac975c39181c`
...
> **Важно:** параметр `iterations` соответствует значению `maxnumber`!<br />
...
— скопируйте и передайте его в запросе ровно в таком виде, включая все символы и параметры (*edk*, *codeChallenge* и т.д.).
  ...
  Цифровая подпись запроса.<br />
  Пример: `4b1cf0e0be0f4e5247e50b0f9a449830f1fbca44c32ff94bc080146815f31a18`
...
После декодирования значения формируется JSON-объект с данными, необходимыми для проверки капчи.
...
(async () => {
  ...
  while (Date.now() - pollStart < maxPollMs) {
    ...
    if (res.status === "ready") {
      ...
      console.log("Full solution from CapMonster Cloud:", fullSolution);
      break;
    }
