...
<TabItem value="proxyless" label="RecaptchaV2EnterpriseTaskProxyless (without proxy)" default className="bordered-panel">
  ...
  **TurnstileTask**
  ...
  Ключ Turnstile (*см. раздел [Как найти все нужные параметры для создания задачи](#как-найти-все-нужные-параметры-для-создания-задачи)*)
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    let turnstileRequest = new TurnstileRequest({
        ...
        websiteKey: "YOUR_WEBSITE_KEY_HERE", // Замените на корректное значение
        data: "YOUR_DATA_HERE",
        pageAction: "managed",
        cloudflareTaskType: "token",
...
async function main() {
    ...
    const turnstileRequest = new TurnstileRequest({
        websiteURL: 'https://example.com/',
        websiteKey: 'xxxxx', // Замените на корректное значение
        cloudflareTaskType: 'cf_clearance',
        proxy,
        htmlPageBase64: 'PGh0bW...h0bWw+',
        userAgent: 'userAgentPlaceholder'
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        {
            WebsiteUrl = "https://example.com",           // URL страницы с капчей
            WebsiteKey = "YOUR_WEBSITE_KEY_HERE",      // Ваш sitekey
            Data = "YOUR_DATA_HERE",
            PageAction = "managed",
            CloudflareTaskType = "token",
            PageData = "YOUR_PAGE_DATA_HERE",
