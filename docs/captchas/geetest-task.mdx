...
<br />
  ...
  <div className="bordered-panel">
    ...
    **GeeTestTask**
    ...
    <p>Динамический ключ.<br />
    Для каждого обращения к API необходимо получать новое значение `challenge`. Если капча уже была загружена на странице, текущее значение `challenge` становится недействительным, и API вернёт [ошибку](../api/api-errors.mdx) `ERROR_TOKEN_EXPIRED`.<br />
Задачи, завершившиеся с ошибкой `ERROR_TOKEN_EXPIRED`, тарифицируются как успешно решённые.<br />
Необходимо проанализировать сетевые запросы и определить запрос, в ответе которого возвращается актуальное значение `challenge`. <br />
Перед каждым созданием задачи на распознавание капчи следует повторно выполнять этот запрос и извлекать из ответа новое значение `challenge`.</p>
  ...
  <details>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        let geetestRequest = new GeeTestRequest({
            ...
            gt: "gt-value",        // Замените на корректное значение
            challenge: "challenge-value", // Замените на корректное значение
            version: 3
        });
        ...
        geetestRequest = new GeeTestRequest({
            websiteURL: "https://example.com/geetest.php", // Замените на корректное значение
            gt: "gt-value",       // Замените на корректное значение
            challenge: "challenge-value", // Замените на корректное значение
            version: 3,
            proxy,
...
<details>
      ...
      async function solveGeeTest() {
          ...
          geetestRequest = new GeeTestRequest({
              websiteURL: "https://example.com/geetest.php", // URL страницы с капчей
              gt: "gt-value",                               // Замените на корректное значение
              challenge: "challenge_challenge",             // Замените на корректное значение
              version: 3,
              proxy,
              userAgent: "userAgentPlaceholder"
          });
  ...
  <details>
    ...
    async def solve_geetest():
        ...
        geetest_request = GeetestRequest(
            websiteUrl="https://example.com", # Замените на корректное значение
            gt="gt-value", # Замените на корректное значение
            challenge="challenge-value", # Замените на корректное значение
            version=3
        )
...
### <span style={{fontSize: '1.5rem'}}>Параметры запроса</span>
  ...
  <div className="bordered-panel">
    ...
    **GeeTestTask**
    ...
    <ParamItem title="userAgent" type="string" />
    User-Agent браузера. <br />
  **Передавайте только актуальный UA от ОС Windows. Сейчас таковым является**: `userAgentPlaceholder` <br />
...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type">
  ...
  <details>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        let geetestRequest = new GeeTestRequest({
            websiteURL: "https://example.com/geetest.php", // URL страницы с капчей
            gt: "captcha-id-value",                               // Замените на корректное значение
            version: "4",
