...
Необходимо проанализировать сетевые запросы и определить запрос, в ответе которого возвращается актуальное значение `challenge`. Перед каждым созданием задачи на распознавание капчи следует повторно выполнять этот запрос и извлекать из ответа новое значение `challenge`.</p>
