...
<br />
  ...
  <div className="bordered-panel">
    ...
    **BinanceTask**
    ...
    Уникальный параметр для раздела вашего сайта. Значение параметра `bizId`, `bizType` или `bizCode`. Его можно взять из трафика (см. описание ниже).<br />
    Пример формата: `login`

    ---

    <ParamItem title="validateId" required type="string" />
    Динамический ключ. Значение параметра `validateId`, `securityId` или `securityCheckResponseValidateId`. Его можно взять из трафика (см. описание ниже).<br />
    Пример формата: `cb0bfefa598b4c3887661fde54ecd57b`
...
<details>
  ...
  document.addEventListener("DOMContentLoaded", async () => {
      ...
      let binanceRequest = new BinanceRequest({
          ...
          validateId: 'cb0bfefa598b4c3887661fde54ecd57b' // Замените на корректное значение
      });
