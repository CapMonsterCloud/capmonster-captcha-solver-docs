...
Если в запросе было указано `"cookieRequired": true`, ответ дополнительно содержит cookies, сгруппированные по доменам. Используйте cookies из объекта `Domains` при последующих запросах к соответствующему домену вместе с результатом решения CAPTCHA.
