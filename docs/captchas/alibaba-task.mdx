...
**Важно:** значение региона влияет на тип возвращаемого токена: при `region: cn` возвращается токен вида `WEB#` (`VOVCI2`), при `region: sgp` – `SG_WEB#` (`U0dfV0VCIZ`).
...
:::info Формат токена зависит от region

Значение `region`, переданное в `metadata` при создании задачи, влияет на формат возвращаемого `deviceToken`:

- `region: cn` – токен вида `WEB#` (`VOVCI2`);
- `region: sgp` – токен вида `SG_WEB#` (`U0dfV0VCIZ`).
