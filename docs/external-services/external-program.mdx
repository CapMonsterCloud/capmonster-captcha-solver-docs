...
Чтобы решить reCAPTCHA, выберите `Util::ReCaptcha2` и укажите `CapMonster Cloud` в поле `Provider`.

![](./images/external-program/aparser.png) 
![](./images/external-program/aparser1.png) 

Чтобы решить стандартные капчи, выберите `Util::Antigate` и укажите `CapMonster Cloud` в поле `Provider`. В поле `Client key` введите ваш API-ключ CapMonster Cloud.
...
1. В вашей программе выберите один из сервисов распознавания капчи, которые мы поддерживаем:
   `Anti-Captcha (v1.0, v2.0), RuCaptcha, RipCaptcha, 2Captcha, BypassCaptcha, DeathByCaptcha`.

2. Затем укажите ваш личный API-ключ CapMonsterCloud в соответствующем поле программы, из которой вы будете отправлять капчи.

> **примечание для DeathByCaptcha:** для идентификации пользователя API DeathByCaptcha использует имя пользователя и пароль, а не API-ключ. В этом случае укажите API-ключ CapMonsterCloud в поле пароля. В поле имени пользователя введите любое значение.

3. Последний шаг: сопоставьте наш IP с другим сервисом, чтобы CapMonster Cloud мог получать капчи от вышеупомянутых сервисов. Для этого выберите вашу операционную систему, откройте спойлер и следуйте инструкции.
...
2. В меню блокнота выберите `Файл` — откройте и укажите путь к файлу hosts в папке *C:\Windows\System32\drivers\etc.* Если в этой папке присутствует несколько файлов с таким именем, открывайте тот, который не имеет никакого расширения.
3. Внесите необходимые изменения в файл hosts и после этого сохраните файл через меню.
:::

Попробуйте зайти на любой из этих доменов. Если все сделано правильно - откроется белая страница. Если у вас не получилось, напишите нам в [службу поддержки](https://helpdesk.zennolab.com/ru/conversation/new), мы поможем вам в настройке!
</details>
...
Теперь попробуйте зайти на любой из этих доменов. Если все сделано правильно - откроется белая страница. Если у вас не получилось, напишите нам в [службу поддержки](https://helpdesk.zennolab.com/ru/conversation/new), мы поможем вам в настройке!
</details>
...
Если у вас не получилось подключить CapMonster Cloud к вашему приложению, пожалуйста, напишите нам в [службу поддержки](https://helpdesk.zennolab.com/ru/conversation/new), мы поможем вам в настройке!
