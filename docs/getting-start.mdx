...
Прежде чем перейти к решению капч, зарегистрируйтесь в [CapMonster Cloud](https://capmonster.cloud/ru/) удобным для вас способом. Затем перейдите в [Личный кабинет](https://dash.capmonster.cloud), где вы найдёте всю необходимую информацию — текущий баланс, API-ключ и статистику по успешно решённым и нерешённым задачам.
...
Для пользователей из России также доступен альтернативный домен API:

```http
https://ru.api.capmonster.cloud
```

Альтернативный домен снижает риск сетевых ошибок и блокировок. Остальные параметры и логика работы API не меняются.
