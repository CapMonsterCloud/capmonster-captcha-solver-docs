...
Для пользователей из России также доступен альтернативный адрес метода:

```http
https://ru.api.capmonster.cloud/createTask
```

Он предназначен для снижения риска сетевых ошибок и блокировки запросов. При его использовании изменяется только адрес метода; параметры запроса, формат `JSON POST`, логика работы API и структура ответа остаются без изменений.
...
Type: `String` <br />
Обязательный: `Да`<br />
Уникальный ключ вашей учетной записи, API ключ (найти можно [здесь](https://capmonster.cloud/Dashboard))

### `task`
Type: `Объект задачи` <br />
Обязательный: `Да`<br />
Массив данных о задаче. Список типов задач капч приведён в [следующем разделе](../../../captchas).
