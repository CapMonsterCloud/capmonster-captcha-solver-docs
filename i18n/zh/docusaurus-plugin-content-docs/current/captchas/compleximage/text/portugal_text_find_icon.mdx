...
请求中必须传递一张 base64 格式的图像，该图像由 5 张按显示顺序水平拼接的图像组成（不包含文本）。
 下面提供了详细的拼接示例。此外，还需要在 `TaskArgument` 参数中指定任务文本，即要选择的图标。
...
<div className="bordered-panel">
    ...
    **recognition**
    ...
    <ParamItem title="TaskArgument（位于 metadata 中）" required type="string" />
    用于指明需要查找哪个图标的任务文本。例如：`"Clique no(a) quadrado."`<br />

    **重要**：任务文本中的某些字符必须经过序列化转义，以避免在处理和解决任务时出现错误。
 例如，对于文本 `"Por favor, clique no(a) avião."`，必须传递字符串 `"Por favor, clique no(a) avi\\u00E3o."`（*参见[自动识别与解决示例](#自动识别与解决示例)章节中的示例*）。
...
<div className="method-panel-full">
	...
	<MethodDescription>
		**请求示例**
		...
		```
		**响应示例**<br/>
		返回的索引是需要单击的图标编号。图标编号从 1 开始，到 5 结束：
```json
{
  "solution": {
    "answer": [4],
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const portugalTextFindIconRequest = new ComplexImageTaskRecognitionRequest({
    imagesBase64: ["your_image_base64"],
    metaData: {
      Task: "portugal_text_find_icon",
      TaskArgument: "Encontre o(a) avi\u00E3o.",
    },
  });
  ...
  const result = await client.Solve(portugalTextFindIconRequest);
...
async function solvePortugalTextFindIcon() {
  const client = CapMonsterCloudClientFactory.Create(
    new ClientOptions({ clientKey: API_KEY }),
  );
...
solvePortugalTextFindIcon().catch(console.error);
```
</details>
...
async def solve_portugal_text_find_icon():
    client = CapMonsterClient(
        options=ClientOptions(api_key=API_KEY)
    )

    # RecognitionComplexImageTaskRequest 不需要代理
    portugal_text_find_icon_request = RecognitionComplexImageTaskRequest(
        imagesBase64=[
            "your_image_base64",
        ],
        metadata={
    ...
    result = await client.solve_captcha(portugal_text_find_icon_request)
    print("Solution:", result)


asyncio.run(solve_portugal_text_find_icon())
```
    </details>
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        // RecognitionComplexImageTaskRequest 不需要代理
        var portugalTextFindIconRequest = new RecognitionComplexImageTaskRequest
        {
            ImagesBase64 = new[]
            {
                "your_image_base64"
        ...
        var result = await cmCloudClient.SolveAsync(portugalTextFindIconRequest);
