...
<div className="method-panel-full">
	...
	<MethodDescription>
		**请求示例**
		...
		```
		**响应示例：**<br/>
		由 `true` 或 `false` 元素组成的值数组，具体取决于图像中的数字是否与目标参数匹配。  
```json
{
  "errorId": 0,
  "status": "ready",
...
此方法通过使用 `ImageToTextTask` 并结合 `bls_text` 模块来实现（*参见章节 [模块名称传递](/docs/captchas/ImageToText/module-name)*）。在该模式下，**每张图片都会作为单独的验证码进行处理**，返回结果为识别出的文本值。
