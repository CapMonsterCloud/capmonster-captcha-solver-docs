...
该对象包含用于解决 Google Enterprise 版 ReCaptcha2 任务的数据。为了确保此类 CAPTCHA 解决方案的通用性，我们需要使用您在目标网站自动填写表单时所使用的所有数据，包括代理、浏览器 UserAgent 和 cookies。这样可以避免 Google 更改其 CAPTCHA 代码时可能出现的问题。
这种类型的 CAPTCHA 与 ReCaptcha v.2 在视觉上没有区别。可以通过 CAPTCHA 代码识别差异，例如在开发者工具的 **Network（网络）** 选项卡中查看请求：
请求 URL（anchor）：`https://www.google.com/recaptcha/enterprise/anchor?ar=1&k=6Lf26sUnAAAAAIKLuWNYgRsFUfmI-3Lex3xT5N-s&co=aHR0cHM6Ly8yY2FwdGNoYS5jb206NDQz&hl=en&v=1kRDYC3bfA-o6-tsWzIBvp7k&size=normal&cb=43r1q2d3mx66`
与普通 CAPTCHA 相比，此类 CAPTCHA 的解决时间可能更长，但获取到的 `g-recaptcha-response` 在 CAPTCHA 解决后仍会继续有效 60 秒。
...
在此示例中，`6LdIFr0ZAAAAAO3vz0O0OQrtAefzdJcWQM2TMYQH` 是 `websiteKey` 参数的值。
    ...
    包含 reCAPTCHA Enterprise 附加参数的对象。

    **对象参数：**

    - `s` — `string`，reCAPTCHA Enterprise 的附加参数，可与 `sitekey` 一起传递给 `grecaptcha.enterprise.render`。
...
<br />
要从 `enterprisePayload` 中获取参数 `s`，请在调用 `grecaptcha.enterprise.render` 之前替换该函数，并从传入的参数对象中提取相应值。该函数会在脚本加载后可用，而 CAPTCHA 通常会立即渲染或由页面事件触发，因此需要提前设置拦截。调用原始函数时可以不传递 `s` 字段。

`apiDomain` 是托管 reCAPTCHA 交互 API 接口的域名，用于验证用户是否已通过 CAPTCHA。
