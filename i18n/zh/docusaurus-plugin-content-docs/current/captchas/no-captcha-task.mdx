...
title: "使用 CapMonster Cloud API 识别和解决 reCAPTCHA v2 — reCAPTCHAV2Task"
description: "了解如何通过 API 配置自动识别，并使用 Capmonster Cloud 中的 NoCaptchaTask 方法解决（绕过）reCAPTCHA v2。"
---
...
以下是 CapMonster Cloud 当前支持的 reCAPTCHA v2 示例：

<ImagesLayout gap="16px" columns={3}>
  <ImageWrap>![](./images/no-captcha/recaptchaV2.png)</ImageWrap>
  <ImageWrap>![](./images/no-captcha/recaptchaV2_1.png)</ImageWrap>
  <ImageWrap>![](./images/no-captcha/recaptchaV2_2.png)</ImageWrap>
</ImagesLayout>
...
## 对象结构
  ...
  <div className="bordered-panel">
    ...
    **RecaptchaV2Task**
    ...
    目标页面上的 ReCaptcha2 标识密钥：`<div class="g-recaptcha" data-sitekey="THIS_ONE"></div>`<br />
    
  大致格式如下：`6Lcg7CMUAAAAANphynKgn9YAgA4tQ2KI_iqRyTwd`

     ---

    <ParamItem title="recaptchaDataSValue" type="string" />
    某些 ReCaptcha2 小部件实现可能会在 ReCaptcha2 的 div 中包含额外的 "data-s" 参数。该参数是一次性令牌，每次解决 ReCaptcha2 时都必须重新获取。
...
可通过以下地址获取最新值：[https://capmonster.cloud/api/useragent/actual](https://capmonster.cloud/api/useragent/actual)。
    ...
    <p>与目标页面交互时需要使用的附加 Cookie。</p>
	<p>**格式**：cookiename1=cookievalue1; cookiename2=cookievalue2</p>

     ---

    <ParamItem title="isInvisible" type="bool" />
    如果验证码为隐形验证码，即具有隐藏的确认字段且没有复选框，则为 true。检测到疑似机器人时会触发额外验证。
...
```
	...
	<TabItem value="proxy" label="RecaptchaV2Task（使用代理时）" className="method-panel">
		<MethodItem>
			```http
			https://api.capmonster.cloud/createTask
			```
...
**Elements** 选项卡：页面上的 `<div class="g-recaptcha">` 元素。复制 *data-sitekey* 属性值，例如：

![sitekey](images/no-captcha/sitekey.png)

**Networks** 选项卡：

打开 **Networks** 选项卡并重新加载包含验证码的页面。找到类似的请求（**k** 的值就是 *data-sitekey*）：

![sitekey1](images/no-captcha/sitekey1.png)

#### recaptchaDataSValue（如果使用）

如果页面中存在 *data-s* 属性，请在 HTML 中找到它：

![datas](images/no-captcha/data-s.png)

#### isInvisible

**Networks** 选项卡：如果验证码是隐形验证码，元素中会包含 *size="invisible"* 属性，例如：

![isinvisible](images/no-captcha/isinvisible.png)
...
async def solve_recaptcha_v2():
    ...
    result = await client.solve_captcha(recaptcha2_request)
