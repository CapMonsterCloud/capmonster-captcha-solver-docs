...
**重要：** `region` 的值会影响返回的 token 类型：当 `region: cn` 时，返回 `WEB#` 类型的 token（`VOVCI2`）；当 `region: sgp` 时，返回 `SG_WEB#` 类型的 token（`U0dfV0VCIZ`）。
...
:::info token 格式取决于 region

创建任务时在 `metadata` 中传递的 `region` 值会影响返回的 `deviceToken` 格式：

- `region: cn` – 返回 `WEB#` 类型的 token（`VOVCI2`）；
- `region: sgp` – 返回 `SG_WEB#` 类型的 token（`U0dfV0VCIZ`）。
