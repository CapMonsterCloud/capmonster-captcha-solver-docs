...
## 获取任务结果的方法
使用 [getTaskResult](../api/methods/get-task-result.mdx) 方法获取 Alibaba 验证码的解决结果。

如果请求中指定了 `"cookieRequired": true`，响应还会包含按域名分组的 cookies。
请将 `Domains` 对象中的 cookies 与 CAPTCHA 识别结果一起用于向相应域名发送的后续请求。
