...
<br />
  ...
  <Tabs>
  <TabItem value="proxy" label="GeeTestTask（使用代理时）" className="bordered-panel">
    ...
    **GeeTestTask**
    ...
    <p>动态密钥。<br />
    每次调用 API 都必须获取新的 `challenge` 值。如果验证码已经在页面上加载，当前 `challenge` 值将失效，API 会返回[错误](../api/api-errors.mdx) `ERROR_TOKEN_EXPIRED`。<br />
以 `ERROR_TOKEN_EXPIRED` 错误结束的任务将按成功解决计费。<br />
需要分析网络请求，并确定哪个请求的响应会返回当前有效的 `challenge` 值。 <br />
每次创建验证码识别任务前，都应重新执行此请求，并从响应中提取新的 `challenge` 值。</p>
...
<Tabs>
<TabItem value="proxyless" label="GeeTestTask（无代理）" default className="method-panel-full">
	<MethodItem>
		```http
  ...
  <details>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        let geetestRequest = new GeeTestRequest({
            websiteURL: "https://example.com/geetest.php", // 包含验证码的页面 URL
            gt: "gt-value",        // 替换为正确的值
            challenge: "challenge-value", // 替换为正确的值
            version: 3
        });
        ...
        geetestRequest = new GeeTestRequest({
            websiteURL: "https://example.com/geetest.php", // 替换为正确的值
            gt: "gt-value",       // 替换为正确的值
            challenge: "challenge-value", // 替换为正确的值
            version: 3,
            proxy,
...
async function solveGeeTest() {
    ...
    geetestRequest = new GeeTestRequest({
        websiteURL: "https://example.com/geetest.php", // 包含验证码的页面 URL
        gt: "gt-value",                               // 替换为正确的值
        challenge: "challenge_challenge",             // 替换为正确的值
        version: 3,
        proxy,
        userAgent: "userAgentPlaceholder"
    });
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        {
            WebsiteUrl = "https://example.com/demo/geetest", // 包含验证码的页面 URL
            Gt = "gt-value",                             // 替换为正确的值
            Challenge = "challenge-value",               // 替换为正确的值
            Version = 3                                       // 验证码版本
        };
