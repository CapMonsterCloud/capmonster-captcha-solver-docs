...
<br />
  ...
  <TabItem value="proxy" label="GeeTestTask (using proxy)" className="bordered-panel">
    ...
    **GeeTestTask**
    ...
    <p> 一个动态密钥。<br />
    对于每一次 API 请求，都需要获取新的 `challenge` 值。如果验证码已经在页面上加载过，当前的 `challenge` 值将失效，API 会返回 [错误](../api/api-errors.mdx) `ERROR_TOKEN_EXPIRED`。<br />
以 `ERROR_TOKEN_EXPIRED` 结束的任务将按成功完成的任务计费。<br />
需要分析网络请求，并确定哪个请求的响应中返回了最新的 `challenge` 值。在每次创建验证码识别任务之前，都应重新执行该请求，并从响应中提取新的 `challenge` 值。</p>
