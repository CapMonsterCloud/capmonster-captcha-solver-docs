...
title: "Binance 验证码自动识别与绕过 | API"
description: "了解如何使用 Capmonster Cloud 自动验证码识别与解决服务的 API 绕过 Binance 验证码！" 
---
...
<br />
  ...
  <Tabs>
  <TabItem value="proxy" label="BinanceTask（使用代理时）" className="bordered-panel">
    ...
    **BinanceTask**
    ...
    您网站相应部分的唯一参数。其值为 `bizId`、`bizType` 或 `bizCode` 参数的值。可从流量中获取（参见下方说明）。<br />
    格式示例：`login`

    ---

    <ParamItem title="validateId" required type="string" />
    动态密钥。其值为 `validateId`、`securityId` 或 `securityCheckResponseValidateId` 参数的值。可从流量中获取（参见下方说明）。<br />
...
```
  ...
  <TabItem value="proxy" label="BinanceTask（使用代理时）" default className="method-panel">
	<MethodItem>
		```http
		https://api.capmonster.cloud/createTask
		```
