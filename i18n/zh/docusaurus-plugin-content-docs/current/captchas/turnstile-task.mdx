...
<div className="bordered-panel">
  ...
  **TurnstileTask**
  ...
  Turnstile 密钥 	<br />
  大致格式应如下：`0x4AAAAAAABUYP0XeMJF0xoy`
...
可通过以下地址获取最新值：[https://capmonster.cloud/api/useragent/actual](https://capmonster.cloud/api/useragent/actual)。
  ...
  可在加载验证码的 callback 函数中找到 `action` 字段

  ---
  
  <ParamItem title="data" type="string" />
  *data* 字段的值可从 `cData` 参数中获取。
...
使用 [getTaskResult](../api/methods/get-task-result.mdx) 方法获取 Turnstile 的解决结果。根据系统负载，您将在 5 到 20 秒内收到响应。
...
|**属性**|**类型**|**说明**|
| :- | :- | :- |
|token|String|在 input 字段中或调用 callback 函数时使用此令牌|
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    let turnstileRequest = new TurnstileRequest({
        websiteURL: "https://yourwebsite.com/page-with-turnstile", // 包含验证码的页面 URL
