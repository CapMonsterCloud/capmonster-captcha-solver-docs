...
<div className="bordered-panel">
    ...
    **captchaAppId**，对应 `aid` 的值。例如，`"websiteKey": "189123456"` 是您网站的唯一参数。可从包含验证码的 HTML 页面或网络流量中获取（参见下文说明）。
    ...
    验证码脚本链接。通常以 `TCaptcha.js` 或 `TCaptcha-global.js` 结尾。可在请求列表中找到（参见下方示例）。
...
## 如何获取 captchaUrl
打开*开发者工具*，转到 **Network** 选项卡，触发验证码并检查网络请求。其中会出现 `TCaptcha.js` 或 `TCaptcha-global.js`，可在其中找到类似的链接： 
