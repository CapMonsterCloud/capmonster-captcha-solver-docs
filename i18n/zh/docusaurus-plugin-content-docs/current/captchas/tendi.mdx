...
sidebar_position: 13
sidebar_label: TenDI
title: "TenDI 验证码自动识别与绕过 — Tencent captcha | API"
description: "了解如何使用 CapMonster Cloud 自动验证码识别与解决服务的 API 绕过 TenDI（Tencent captcha）验证码！" 
---
...
<div className="bordered-panel">
    ...
    **captchaAppId**，对应 `aid` 的值。例如，`"websiteKey": "189123456"` 是您网站的唯一参数。可从包含验证码的 HTML 页面或网络流量中获取（参见下文说明）。

    ---

    <ParamItem title="captchaUrl（位于 metadata 中）" type="string" />
    CAPTCHA 脚本链接。文件通常名为 `TCaptcha.js` 或 `TCaptcha-global.js`。对于第二个版本，也可能使用名为 `TJNCaptcha*.js` 和 `TJCaptcha*.js` 的文件，其中 `*` 表示任意字符串或空字符串，例如 `-global`。可在网络请求中找到该脚本（参见下方示例）。
...
## 如何获取 captchaUrl
打开*开发者工具*，转到 **Network** 选项卡，触发验证码并检查网络请求。在已加载的文件中找到 `TCaptcha.js` 或 `TCaptcha-global.js`：

![](./images/tendi/tendi_captchaUrl.png)

对于第二个版本，也可能使用名为 `TJNCaptcha*.js` 和 `TJCaptcha*.js` 的文件：

![](./images/tendi/tendi_captchaUrl-1.png)

文件名中的 `*` 表示任意字符串或空字符串。例如，`TJNCaptcha*.js` 可以匹配 `TJNCaptcha.js` 或 `TJNCaptcha-global.js`。
