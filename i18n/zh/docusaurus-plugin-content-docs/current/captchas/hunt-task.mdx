...
可在包含 Hunt 验证码的页面中通过 DevTools（**Network** 或 **Elements** 选项卡）查找该链接。  
请使用关键字 `hd-api` 或 `api.js` 进行搜索。

---

<ParamItem title="data（位于 metadata 中）" type="string" />
选择验证码解决模式时必须指定 `data`（详情见下文）
...
4. 向 API 发送后续请求时，必须在 `Cookie` 请求头中将 Cookie 传回服务器，以便服务器确认请求属于同一会话。

5. 所有请求都必须在同一会话中使用相同的 IP（或代理）和 User-Agent。否则，服务器可能会将请求视为可疑并要求重新验证。

6. 某些 Cookie 有时会在客户端生成（例如页面中的 JavaScript）。在自动化或测试过程中，有时需要从浏览器中提取 Cookie，或以类似方式生成它们。
...
api_headers = {
    ...
    "content-type": "application/vnd.api+json",
    "origin": BASE_URL,
    "referer": BASE_URL + "/en", # 替换为网站当前有效的 referer
    "sec-ch-ua": '"Not:A-Brand";v="99", "Google Chrome";v="150", "Chromium";v="150"',
    "sec-ch-ua-mobile": "?0",
    "sec-ch-ua-platform": '"Windows"',
    "sec-fetch-dest": "empty",
...
# ================= CAPMONSTER CLOUD =================
# 用于操作 CapMonster Cloud 的函数（创建任务并等待验证码解决结果）
