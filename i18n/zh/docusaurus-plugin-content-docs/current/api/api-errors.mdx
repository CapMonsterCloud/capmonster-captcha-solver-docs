...
请求在语法上是正确的，但包含无法创建任务的无效数据。例如，必填字段缺失、字段值错误或数据已过期，导致无法完成验证码识别。`errorDescription` 字段会说明哪些请求字段存在问题以及如何修正。请检查任务参数，确认提交的数据仍然有效，并根据 `errorDescription` 修正相应字段。如果仍无法自行解决问题，请联系[技术支持](https://helpdesk.zennolab.com/conversation/new)。
