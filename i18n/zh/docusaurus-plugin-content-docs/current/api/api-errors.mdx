...
如果错误仍然存在且无法自行解决，请通过[表单](https://helpdesk.zennolab.com/zh-cn/conversation/new)、我们在[网站](https://capmonster.cloud/)和[文档](https://docs.capmonster.cloud/)中的聊天，或在 [Telegram 频道](https://t.me/capmonstercloud) 联系支持，并附上：
...
代理的登录名或密码包含无效字符 %. 请在不包含此字符的情况下提供代理凭据并重试请求。
...
服务暂时不可用。当回调（callback）服务不可用，或 [getTaskResult](./methods/get-task-result.mdx) 可用性检查失败时，将返回此错误。为了避免用户为无法获取结果的任务付费，在这些情况下将禁止创建新任务。建议：请稍后重试。如果使用了 callback，请确保指定的回调 URL 可以正常访问。

### `INVALID TASK`
API 错误代码：`ERROR_INVALID_TASK`<br />
请求在语法上是正确的，但包含无法创建任务的无效数据。例如，必填字段缺失、字段值错误或数据已过期，导致无法完成验证码识别。`errorDescription` 字段会说明哪些请求字段存在问题以及如何修正。请检查任务参数，确认提交的数据仍然有效，并根据 `errorDescription` 修正相应字段。如果仍无法自行解决问题，请联系[技术支持](https://helpdesk.zennolab.com/zh-cn/conversation/new)。
