...
To solve a reCAPTCHA, select `Util::ReCaptcha2` and choose `CapMonster Cloud` in the `Provider` field.

![](./images/external-program/aparser.png) 
![](./images/external-program/aparser1.png) 

To solve standard captchas, select `Util::Antigate` and choose `CapMonster Cloud` in the `Provider` field. In the `Client key` field, enter your CapMonster Cloud API key.
...
2. Then specify your personal API key for CapMonsterCloud in the relevant field in your program from which you will send captchas.
...
3. Last step: match our IP with another service so CapMonster Cloud can get captchas from the aforementioned services. For that, select your operating system, hit the spoiler, and follow the instruction.
...
2. In the Notepad upper menu, click `File` — open and specify the path to `hosts`: *C:\Windows\System32\drivers\etc.* If there are several files with this name in the folder, open the file that has no extension.
3. Make changes to `hosts` and then save the file in the menu.
:::

Try visiting any of these domains. If everything is correct, a white page will open. If you couldn't do it, contact [support](https://helpdesk.zennolab.com/en-us/conversation/new), we will help you with all configurations!
</details>
...
If you couldn't connect CapMonster Cloud to your app, contact [support](https://helpdesk.zennolab.com/en-us/conversation/new), we will help you with all configurations.
