...
**Important:** the region value affects the type of token returned: with `region: cn`, a `WEB#` token (`VOVCI2`) is returned; with `region: sgp`, an `SG_WEB#` token (`U0dfV0VCIZ`) is returned.
...
:::info Token format depends on region

The `region` value passed in `metadata` when creating the task affects the format of the returned `deviceToken`:

- `region: cn` – `WEB#` token (`VOVCI2`);
- `region: sgp` – `SG_WEB#` token (`U0dfV0VCIZ`).
