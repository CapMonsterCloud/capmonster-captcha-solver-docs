...
Use the value `true` only for websites that use **Alibaba WAF** or **Amazon WAF** and require cookies after the verification has been successfully completed.
...
For some websites that use **Alibaba WAF** or **Amazon WAF**, cookies must be obtained after the CAPTCHA has been successfully solved. These cookies are then used in subsequent requests.
...
If `"cookieRequired": true` was specified in the request, the response additionally contains cookies grouped by domain.
Use the cookies from the `Domains` object in subsequent requests to the corresponding domain together with the CAPTCHA solution.
