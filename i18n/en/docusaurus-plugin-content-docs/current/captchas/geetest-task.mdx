...
<br />
  ...
  <div className="bordered-panel">
    ...
    **GeeTestTask**
    ...
    The URL of the page where the captcha is initialized. The correct URL is usually passed in the Referer header when making a request to `https://api-na.geetest.com/gettype.php`. For example, you may be on the page `https://example.com#login`, while the actual captcha initialization is performed on `https://example.com`.
...
<p>Dynamic key.<br />
A new `challenge` value must be obtained for each API request. If the captcha has already been loaded on the page, the current `challenge` value becomes invalid, and the API will return the [error](../api/api-errors.mdx) `ERROR_TOKEN_EXPIRED`.<br />
Tasks that end with the `ERROR_TOKEN_EXPIRED` error are billed as successfully solved tasks.<br />
You need to analyze the network requests and identify the request whose response contains the current `challenge` value. Before creating each captcha-solving task, this request should be executed again and a new `challenge` value should be extracted from the response.</p>
  ...
  <details>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        let geetestRequest = new GeeTestRequest({
            ...
            gt: "gt-value",        // Replace with the correct value
            challenge: "challenge-value", // Replace with the correct value
            version: 3
        });
  ...
  <details>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        let geetestRequest = new GeeTestRequest({
            ...
            gt: "captcha-id-value",  // Replace with the correct value
            version: "4",
