...
Below are examples of reCAPTCHA v2 Enterprise variants supported by CapMonster Cloud:
...
## Request parameters
  ...
  <div className="bordered-panel">
    ...
    **RecaptchaV2EnterpriseTask**
    ...
    The reCAPTCHA site key on the target page.<br />
For example, the `websiteKey` value can be found in the page’s HTML:

`<div class="g-recaptcha" data-sitekey="THIS_ONE"></div>`

or in the URL specified in the `src` attribute of an `iframe` element:

`<iframe title="reCAPTCHA" src="...;k=6LdIFr0ZAAAAAO3vz0O0OQrtAefzdJcWQM2TMYQH&amp;...">`

In this example, `6LdIFr0ZAAAAAO3vz0O0OQrtAefzdJcWQM2TMYQH` is the value of the `websiteKey` parameter.
...
<br/>*`grecaptcha.execute('site_key', {action:'login_test'})`*.</p>
    ...
    Object containing additional reCAPTCHA Enterprise parameters.

    **Object parameters:**

    - `s` — `string`, an additional reCAPTCHA Enterprise parameter that may be passed to `grecaptcha.enterprise.render` together with the `sitekey`.
...
```
    ...
    <p>Domain address from which to load reCAPTCHA Enterprise. For example: <br />- [www.google.com](http://www.google.com)<br />- [www.recaptcha.net](http://www.recaptcha.net)</p>
...
<br />
To obtain the `s` parameter from `enterprisePayload`, replace the `grecaptcha.enterprise.render` function before it is called and extract the value from the parameters object passed to it. The function becomes available after the script loads, and the CAPTCHA is usually rendered immediately or on a page event, so the interception must be set up in advance. The original function can be called without the `s` field.
