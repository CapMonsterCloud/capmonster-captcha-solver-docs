...
<br/>*`grecaptcha.execute('site_key', {action:'login_test'})`*.</p>
    ...
    Some reCAPTCHA Enterprise implementations may include an additional `s` parameter passed to `grecaptcha.enterprise.render` together with the `sitekey`. For example, the value `2JvUXHNTnZl1Jb6WEvbDyB...ugQA`:
