...
<div className="bordered-panel">
    ...
    **captchaAppId** corresponde ao valor de `aid`. Por exemplo, `"websiteKey": "189123456"` é um parâmetro exclusivo do seu site. Ele pode ser obtido na página HTML que contém o captcha ou no tráfego de rede (veja a descrição abaixo).
    ...
    Link para o script do CAPTCHA. Normalmente, o arquivo se chama `TCaptcha.js` ou `TCaptcha-global.js`. Para a segunda versão, também podem ser usados arquivos chamados `TJNCaptcha.js` ou `TJCaptcha.js`. O script pode ser encontrado entre as requisições de rede (veja o exemplo abaixo).
...
Para a segunda versão, também podem ser usados arquivos chamados `TJNCaptcha.js` ou `TJCaptcha.js`:
