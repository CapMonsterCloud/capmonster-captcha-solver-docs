...
<TabItem value="proxyless" label="RecaptchaV2EnterpriseTaskProxyless (sem proxy)" default className="bordered-panel">
  ...
  **TurnstileTask**
  ...
  Chave do site Turnstile (*veja a seção [Como encontrar todos os parâmetros necessários para a criação da tarefa](#como-encontrar-todos-os-parâmetros-necessários-para-a-criação-da-tarefa)*)
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    let turnstileRequest = new TurnstileRequest({
        ...
        websiteKey: "YOUR_WEBSITE_KEY_HERE", // Substitua pelo valor correto
        data: "YOUR_DATA_HERE",
        pageAction: "managed",
        cloudflareTaskType: "token",
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        {
            WebsiteUrl = "https://example.com",     // URL da página com a captcha        
            WebsiteKey = "xxxxx",  // Substitua pelo valor correto     
            CloudflareTaskType = "cf_clearance",                                    
...
class Program
{
    ...
    static async Task<string> SolveTurnstile(string htmlBase64)
    {
        ...
        {
            throw new Exception($"Erro do CapMonster SDK: {ex.Message}", ex);
        }
    }
