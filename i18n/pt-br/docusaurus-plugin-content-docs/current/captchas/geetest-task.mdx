...
É necessário analisar as requisições de rede e identificar a solicitação cuja resposta contém o valor atual de `challenge`. Antes de criar cada tarefa de resolução de captcha, essa solicitação deve ser executada novamente, e um novo valor de `challenge` deve ser extraído da resposta.
    </p>
