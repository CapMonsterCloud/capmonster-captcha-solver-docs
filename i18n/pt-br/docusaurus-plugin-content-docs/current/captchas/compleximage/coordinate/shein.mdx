...
<div className="method-panel-full">
    ...
    A resposta contém as coordenadas nas quais é necessário clicar em uma determinada ordem.
      ```json
      {
        "solution": {
          "answer": [
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const sheinRequest = new ComplexImageTaskRecognitionRequest({
  ...
  const result = await client.Solve(sheinRequest);
...
solveShein().catch(console.error);
```
</details>
...
async def solve_shein():
    ...
    result = await client.solve_captcha(shein_request)
    print("Solution:", result)


asyncio.run(solve_shein())
```
    </details>
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        // Não é necessário usar proxy para RecognitionComplexImageTaskRequest
        var sheinRequest = new RecognitionComplexImageTaskRequest
        {
            ImagesBase64 = new[]
            {
                "your_image_base64"
        ...
        var result = await cmCloudClient.SolveAsync(sheinRequest);
