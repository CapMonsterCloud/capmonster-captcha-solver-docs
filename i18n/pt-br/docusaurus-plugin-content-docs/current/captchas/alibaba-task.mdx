...
Se `"cookieRequired": true` tiver sido especificado na solicitação, a resposta também conterá cookies agrupados por domínio.

Use os cookies do objeto `Domains` nas solicitações subsequentes ao domínio correspondente, juntamente com a solução do CAPTCHA.
