...
**Importante:** o valor de `region` afeta o tipo de token retornado: com `region: cn`, é retornado um token do tipo `WEB#` (`VOVCI2`); com `region: sgp`, é retornado um token do tipo `SG_WEB#` (`U0dfV0VCIZ`).
...
:::info O formato do token depende de region

O valor de `region`, enviado em `metadata` ao criar a tarefa, afeta o formato do `deviceToken` retornado:

- `region: cn` – token do tipo `WEB#` (`VOVCI2`);
- `region: sgp` – token do tipo `SG_WEB#` (`U0dfV0VCIZ`).
