...
<br/>*`grecaptcha.execute('site_key', {action:'login_test'})`*.</p>
    ...
    Algumas implementações do reCAPTCHA Enterprise podem conter um parâmetro adicional `s`, transmitido para `grecaptcha.enterprise.render` juntamente com `sitekey`. Por exemplo, o valor `2JvUXHNTnZl1Jb6WEvbDyB...ugQA`:
  ...
  <details>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      let recaptchaV2EnterpriseRequest = new RecaptchaV2EnterpriseRequest({
        ...
        websiteKey: "your-website-key",             // Substitua por um valor válido
        enterprisePayload: {
          s: "your-value",   // Parâmetro opcional do Enterprise
        }
      });
