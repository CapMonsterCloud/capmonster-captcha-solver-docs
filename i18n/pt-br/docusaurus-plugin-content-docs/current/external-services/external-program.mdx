...
title: Como conectar o CapMonster Cloud a um programa?
---
...
Para resolver um reCAPTCHA, selecione `Util::ReCaptcha2` e escolha `CapMonster Cloud` no campo `Provider`.

![](./images/external-program/aparser.png) 
![](./images/external-program/aparser1.png) 

Para resolver captchas padrão, selecione `Util::Antigate` e escolha `CapMonster Cloud` no campo `Provider`. No campo `Client key`, insira sua chave de API do CapMonster Cloud.
...
1. No seu programa, selecione um dos serviços de resolução de captcha compatíveis:
   `Anti-Captcha (v1.0, v2.0), RuCaptcha, RipCaptcha, 2Captcha, BypassCaptcha, DeathByCaptcha`.

2. Em seguida, especifique sua chave de API pessoal do CapMonsterCloud no campo correspondente do programa a partir do qual você enviará captchas.

> **observação para DeathByCaptcha:** para identificar um usuário, a API do DeathByCaptcha usa nome de usuário e senha, não uma chave de API. Nesse caso, especifique a chave de API do CapMonsterCloud no campo de senha. Insira qualquer valor no campo de nome de usuário.

3. Última etapa: associe nosso IP a outro serviço para que o CapMonster Cloud possa receber captchas dos serviços mencionados acima. Para isso, selecione seu sistema operacional, abra o spoiler e siga as instruções.
<details>
...
1. Comece a digitar `Bloco de Notas` na barra de pesquisa da barra de tarefas. Quando o resultado for encontrado, clique nele com o botão direito do mouse e clique em `Executar como administrador`.

  ![](./images/external-program/Aspose.Words.65ffb1d6-0a55-415c-84ac-f87a8022a7cc.008.png)

2. No menu superior do Bloco de Notas, clique em `Arquivo` — abrir e especifique o caminho para `hosts`: *C:\Windows\System32\drivers\etc.* Se houver vários arquivos com esse nome na pasta, abra o arquivo que não tem extensão.
3. Faça alterações no arquivo `hosts` e depois salve o arquivo no menu.
:::

Tente acessar qualquer um desses domínios. Se tudo estiver correto, uma página branca será aberta. Se você não conseguir fazer isso, entre em contato com o [suporte](https://helpdesk.zennolab.com/en-us/conversation/new), nós ajudaremos você com todas as configurações!
</details>
...
Tente visitar qualquer um desses domínios. Se tudo estiver correto, uma página em branco será aberta. Se você não conseguiu, entre em contato com [suporte](https://helpdesk.zennolab.com/en-us/conversation/new), nós ajudaremos com todas as configurações.
</details>
...
Se você não conseguiu conectar o CapMonster Cloud ao seu aplicativo, entre em contato com [suporte](https://helpdesk.zennolab.com/en-us/conversation/new), nós ajudaremos com todas as configurações.
